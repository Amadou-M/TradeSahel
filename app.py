...
from tradesahel_db import (
    ...
    ajouter_note, charger_notes, supprimer_note,
...
init_db()  # crée les tables si besoin (idempotent)
...
def preparer_df(df_raw: pd.DataFrame) -> pd.DataFrame:
    """Calcule CA, marge et colonnes temporelles depuis un DataFrame brut."""
    df = df_raw.copy()
    ...
    if "type_commerce" not in df.columns: df["type_commerce"] = "Commerce Général"
    if "boutique_nom"  not in df.columns: df["boutique_nom"]  = "Principal"
    return df

def generer_pdf(boutique, username, choix, devise, total_ca, total_reel,
...
# ── Import fichiers avec anti-doublon MD5 ─────────────────
st.sidebar.markdown("---")
...
# Charger les données depuis la BDD (persistées)
...
has_stock = "stock" in df_complet.columns and df_complet["stock"].notna().any()
...
with tabs[0]:
    ...
    if "produit" in df_f.columns:
        ...
        if has_marge and "Marge (%)" in det.columns:
            fmt["Marge brute"] = "{:,.0f}"; fmt["Marge (%)"] = "{:.1f}%"
...
# ══════════════════════════════════════════════════════════
# TAB 3 — OBJECTIFS (persistés en BDD)
# ══════════════════════════════════════════════════════════
with tabs[3]:
    ...
    if grp_col in df_complet.columns:
        ...
        rows_obj = []
        for e in entites:
            r = ca_r.get(e,0); o = objectifs.get(e,0)
...
with tabs[4]:
    ...
    if not has_stock:
        st.info("Ajoutez une colonne **stock** dans votre fichier CSV pour activer cette section.")
    else:
        seuil = st.slider("🚨 Seuil d'alerte stock", 1, 100, 10)
        df_sk = df_complet.dropna(subset=["stock"])
        if "produit" in df_sk.columns:
            df_last = df_sk.sort_values("date").groupby("produit")[["stock"]].last().reset_index()
            df_last["stock"]  = pd.to_numeric(df_last["stock"], errors="coerce")
...
with tabs[6]:
    ...
    if prompt_chat := st.chat_input("Posez votre question sur vos ventes, stocks, produits..."):
        st.session_state.chat_history.append(("user", prompt_chat))
            ...
            if not api_key:
                ...
                with st.spinner("L'IA analyse..."):
                    try:
                        ...
                        rep = f"Erreur IA : {e}"; st.error(rep)
            st.session_state.chat_history.append(("assistant", rep))
    if st.button("🗑️ Effacer la conversation"):
        st.session_state.chat_history = []; st.rerun()

# ══════════════════════════════════════════════════════════
# TAB 7 — RAPPORT IA (sauvegardé en BDD)
# ══════════════════════════════════════════════════════════
with tabs[7]:
...
with tabs[8]:
    ...
    if len(valeurs) >= 2:
        ...
        if ca_a > 0:
            ...
            if var <= -seuil_baisse: alertes.append(("🔴",f"Baisse CA : {var:.1f}% entre {valeurs[-2]} et {valeurs[-1]}","error"))
            elif var >= 15:          alertes.append(("🟢",f"Hausse CA : +{var:.1f}% entre {valeurs[-2]} et {valeurs[-1]}","success"))
    if "type_commerce" in df_f.columns:
        cs = df_f.groupby("type_commerce")["chiffre_affaires"].sum()
        moy = cs.mean()
        for s,c in cs.items():
            if c < moy*0.4: alertes.append(("⚠️",f"{s} en sous-performance ({c:,.0f} {devise})","warning"))
    ...
    if has_stock and "produit" in df_complet.columns:
...
# ══════════════════════════════════════════════════════════
# TAB 9 — HISTORIQUE IA (depuis BDD, persisté)
# ══════════════════════════════════════════════════════════
with tabs[9]:
    ...
    if not historique:
        ...
        for h in historique:
            with st.expander(f"📄 {h['type_rapport']} — {h['date']}"):
                ...
                with col_dl:
                    st.download_button("⬇️ Télécharger (.txt)", data=h["contenu"].encode("utf-8"),
                        file_name=f"rapport_{h['date'][:10]}.txt", mime="text/plain", key=f"dl_{h['id']}")
                with col_del:
                    if st.button("🗑️ Supprimer", key=f"del_{h['id']}"):
                        supprimer_rapport_ia(h["id"]); st.rerun()

# ══════════════════════════════════════════════════════════
# TAB 10 — NOTES (persistées en BDD)
# ══════════════════════════════════════════════════════════
with tabs[10]:
    ...
    for n in notes:
        ...
        with col_n:
            prio    = n["priorite"]
            bg_col  = cls_map.get(prio, VERT_CLAIR)
            brd_col = brd_map.get(prio, "#BBF7D0")
            html_n  = (f"<div style='background:{bg_col};border:1px solid {brd_col};"
                       f"border-radius:8px;padding:10px 14px;margin-bottom:6px;'>"
                       f"<div style='font-size:10px;color:#64748B;margin-bottom:4px;'>{n['date']} · {prio.upper()}</div>"
                       f"<div style='font-size:13px;'>{n['contenu']}</div></div>")
            st.markdown(html_n, unsafe_allow_html=True)
        with col_d:
            if st.button("🗑️", key=f"del_note_{n['id']}"):
                supprimer_note(n["id"]); st.rerun()

# ══════════════════════════════════════════════════════════
# TAB 11 — GESTION DES DONNÉES
# ══════════════════════════════════════════════════════════
with tabs[11]:
    ...
    s1.metric("📁 Fichiers importés", stats["nb_fichiers"])
    s2.metric("📊 Lignes en base",    stats["nb_ventes"])
    s3.metric("🤖 Rapports IA",       stats["nb_rapports"])
    s4.metric("📅 Dernier import",    stats["derniere_import"])
    ...
    with cx1:
        ...
        if st.button("⬇️ Générer PDF", use_container_width=True):
            try:
                ...
                pdf_buf = generer_pdf(boutique, username, choix, devise,
    ...
    with cx2:
        ...
        if st.button("⬇️ Générer Excel", use_container_width=True):
            ...
            with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                ...
                for ci,h in enumerate(df_complet.columns): ws1.write(1,ci,h,fmh)
                ...
                if "produit" in df_f.columns:
                    ...
                    for ci,h in enumerate(pp.columns): ws2.write(1,ci,h,fmh)
                if "type_commerce" in df_f.columns:
                    ...
                    for ci,h in enumerate(ps.columns): ws3.write(1,ci,h,fmh)
            output.seek(0)
